...
final class OpenAIClient {
    ...
    private func buildInput(history: [ChatMessage], newText: String, newImages: [UIImage], extraContext: String?) throws -> [[String: Any]] {
        ...
        let system = "Ты помощник для трекинга еды. Отвечай КРАТКО и СТРУКТУРИРОВАННО, без длинных абзацев. Веб-поиск используй ТОЛЬКО когда он реально нужен: ресторан/бренд/готовое блюдо, низкая уверенность, спорные значения, запрос пользователя на источник/проверку. Если блюдо обычное и уверенность достаточная — НЕ используй веб-поиск. Не выдумывай значения. Формат ответа всегда такой:\n\nИтог: <1 строка>\nКалории: <число> ккал\nБ: <г>  Ж: <г>  У: <г>\nИсточник: <коротко, 1-2 домена максимум или \"без веб-поиска\">\nУверенность: <высокая|средняя|низкая>\n\nПравила: максимум 6-8 строк, без длинных объяснений, без повторов, без копирования больших кусков из источников. Если данных мало — задай уточняющие вопросы.\n\nПосле обычного ответа ВСЕГДА добавляй служебный блок в конце в одной строке: <REGISTRATION_JSON>{...}</REGISTRATION_JSON>. JSON-схема: {\"shouldRegister\": boolean, \"action\": \"add\"|\"update_last\"|\"update_by_name\", \"targetName\": string|null, \"items\": [{\"name\": string, \"calories\": number, \"protein\": number, \"fat\": number, \"carbs\": number}]}. shouldRegister=true только если данных достаточно для регистрации. Если пользователь просит исправить НЕ последнюю запись за сегодня — используй action=update_by_name и targetName. Если данных мало: shouldRegister=false и items=[]."
